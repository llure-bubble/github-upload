...
class Kioken_Init {
	...
	function kioken_register_scripts_both_ends() {
		//jarallax
		wp_register_script( 'jarallax', KK_BLOCKS_URL . 'dist/vendor/jarallax/dist/jarallax.min.js', array(), '1.12.0', true );
		wp_register_script( 'resize-observer-polyfill', KK_BLOCKS_URL . 'dist/vendor/resize-observer-polyfill/ResizeObserver.global.min.js', array(), '1.5.0', true );
    wp_register_script( 'jarallax-video', KK_BLOCKS_URL . 'dist/vendor/jarallax/dist/jarallax-video.min.js', array( 'jarallax' ), '1.10.7', true );
	...
	function kioken_gutenberg_editor_assets() {
		...
		if ( current_user_can( apply_filters( 'kioken_blocks_admin_role', 'manage_options' ) ) ) {
				$userrole = 'admin';
			} else if ( current_user_can( apply_filters( 'kioken_blocks_editor_role', 'delete_others_pages' ) ) ) {
				$userrole = 'editor';
			} else if ( current_user_can( apply_filters( 'kioken_blocks_author_role', 'publish_posts' ) ) ) {
				$userrole = 'author';
			} else if ( current_user_can( apply_filters( 'kioken_blocks_contributor_role', 'edit_posts' ) ) ) {
		...
		// Styles.
		wp_enqueue_style( 'kioken-blocks-style-css', KK_BLOCKS_URL . 'dist/blocks.style.build.css', array(), KK_BLOCKS_VERSION );
		wp_enqueue_style( 'kioken-blocks-editor-css', KK_BLOCKS_URL . 'dist/blocks.editor.build.css', array( 'wp-edit-blocks' ), KK_BLOCKS_VERSION );

		if ( get_option( 'kiokenblocks_editor_enhanced', false ) ) {
				wp_enqueue_style( 'kioken-blocks-limited-margins-css', KK_BLOCKS_URL . 'dist/editor-overrides.css', array( 'wp-edit-blocks' ), KK_BLOCKS_VERSION );
			}
	}
	...
	public static function browser_body_classes($classes) {
			...
			$browsers = ['is_iphone', 'is_chrome', 'is_safari', 'is_NS4', 'is_opera', 'is_gecko', 'is_lynx', 'is_IE', 'is_edge'];
	...
	public static function kio_blocks_init_post_meta() {
		register_post_meta('', 'kk_blocks_editor_width', array( 'show_in_rest' => true, 'single' => true, 'type' => 'string', ));
	}

	public static function kioken_init_hook__premium_only() {
			define( 'KK_BLOCKS_ZENGIN', true );
			define( 'KK_BLOCKS_PRO', true ); // used for pro badge in the settings page
	}
